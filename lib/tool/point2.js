...
* 用户服务调度类
*/
define("point2",["jquery","map","leaflet","statictable","config"],function(j,m,i,st,conf){
	var map = m.Ly.map, g, labels=[], infoLabel={};
	function a(treeNode){	
	    return b(treeNode);
	}
	function b(treeNode){
		...
		for(var n in data){
			var o = data[n];
			o.fillColor = conf.CONFIG.getColor(0);
	...
	function c(o,treeNode){
		...
		label.setContent("<div style=\"position: absolute;left:-"+Math.ceil(o.width/2)+"px;opacity: 0.7;background:"+o.fillColor+";width:"+o.width+"px;height:"+o.width+"px;line-height:"+o.width+"px;border-radius: "+Math.ceil(o.width/2)+"px;color:#ffffff;margin-left:2px;font-weight:normal;font-size:10px;text-align: center;font-family:\"微软雅黑 \"\">"
		+"</div>");
