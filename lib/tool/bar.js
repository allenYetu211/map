 ...
 * 用户服务调度类
 */
define("bar", ["jquery", "map", "leaflet", "echarts", "config", "common"], function(j, m, i, ec, conf, common) {
    var map = common.map,
        labels = [],
        infoLabel = {},
        CONFIG = conf.CONFIG,
    ...
    function b(data) {
        ...
        for (var n in data) {
			var o = data[n];
			if( o.hasOwnProperty('center') ){
				var index = div_index;
				var label = createLable(index, o);
				labels.push(label);
				c(o.option, "chart_bar" + index);
				div_index++;
			}
        }
	...
	function createLable(n, o){
		...
		label.setContent("<div id=\"chart_bar" + n + "\" style=\"position: absolute;left:-" + offsetVal + "px;top:-" + offsetVal + "px;width:" + o.width + "px;height:" + o.height + "px;text-align: center;\"></div>");
		...
		label.on("mouseover", function(e) {
			...
			for (var k in info) {
				...
				// c += "<span>" + getLabelValue(info[k]).label + " : " + getLabelValue(info[k]).F_NAME + "</span><br/>";
