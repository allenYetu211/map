define("map", ["leaflet", "jquery", "common", "leafletmeasure"],
function(L, e, common) {
	...
	MapHelper.prototype.enlarge = function(){
		var _this = this, z = _this.map.getZoom();
		_this.map.setZoom(++z);
		return _this;
	}
	...
	MapHelper.prototype.addMarkerPop = function(e, datas){
		var _this = this, latlng = e.latlng;
		var title = datas.title,data = datas.data,
	...
	MapHelper.prototype.mapZoomChange = function(_mapNarrow){
		var _this = this, layers = common.themeLayersList;
		var _oldzoom = false;
	...
	MapHelper.prototype.Measure = function(opt){
		var _this = this, map = _this.map;
		$("#"+_this.mapDiv).css("cursor","url(\"lib/leaflet/images/Line.cur\"), auto");
		//this.cancelMagnify();
		//this.cancelShrink();
		var map = this.map;
			...
			if(measurePoints.length > 0) {
				if(e.latlng.lat != measurePoints[measurePoints.length - 1].lat && e.latlng.lng != measurePoints[measurePoints.length - 1].lng){
					...
					if (measurePoints.length == 2) {
						lines = new L.Polyline(measurePoints, {"color": color, "weight": 1});
						lines.addTo(layer);
					}
					var s = L.circleMarker(e.latlng, {color: color});
					s.setRadius(1);
					if(measurePoints.length >= 2){
						var d = measurePoints[measurePoints.length - 1].distanceTo(measurePoints[measurePoints.length - 2]);
						totalDistance += d;
						var txt = (d / 1000).toFixed(3) + "（km）";//, className: "leaflet-label-tffq"
...
//					s.bindLabel("起点", {noHide: true, direction: 'right'});
		...
		var mm = function(e){
			if (start) {
				...
				tempLine = new L.Polyline([measurePoints[measurePoints.length - 1], e.latlng], {"color": color, "weight": 1});
		...
		var dc = function(e){
			if (measurePoints.length > 1) {
				if(markers.length > 1) {
					...
					var lab = m.getLabel();
					var tt = document.createTextNode(m.getLabel()._content + "  总长度：" + (totalDistance / 1000).toFixed(3) + "（km）");
	...
	MapHelper.prototype.areaMeasure = function(opt){
		var _this = this, map = _this.map;
		var areaMeasure = {
				points:[],
				color: opt.color,
				polygon: null,
				lab: null,
				close:function(e, area){
					areaMeasure.lab = new L.Label({noHide: true, direction: 'right', clickable: true, className: "leaflet-label-tffq"});
					areaMeasure.lab.setLatLng(e.latlng);
					map.addLayer(areaMeasure.lab);
					var spanArea = document.createElement("span");
					spanArea.innerHTML = "面积: " + area;
					var span = document.createElement("span");
					span.innerHTML = "【关闭】";
					...
					areaMeasure.lab._container.appendChild(div);
					L.DomEvent.addListener(span,"click",function(){
						areaMeasure.destory();
					});
				},
				...
				mousemove:function(e){
					...
					areaMeasure.polygon = L.polygon(areaMeasure.points, {showMeasurements: true, color: areaMeasure.color, weight: 1});
				...
				dblclick:function(e){ // 双击结束
					// 添加结束信息
					areaMeasure.close(e, areaMeasure.polygon.options.area);
					$("#"+_this.mapDiv).css("cursor","");
				...
				destory:function(){
					if(areaMeasure.polygon)
						map.removeLayer(areaMeasure.polygon);
					if(areaMeasure.lab)
						map.removeLayer(areaMeasure.lab);
				}
			 }
