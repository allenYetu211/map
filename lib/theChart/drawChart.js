...
var cblin = {
    ...
    var initTerms = {
        ...
        callBackMapData: [],
        init: function(_level = '0', _code = '330283103', bench = false) {
            var handleKeyValues = []
            var location = ''
            var locationClass = ''
            var chartType = ''
            for (let i = 0; i < cblin._indicator.indicators.length; i++) {
                ...
                if ($(location).find('.mouseMove').hasClass('draw_' + cblin._indicator.indicators[i].indicatorID)) {
                    ...
                    $(location).append("<div class='information " + locationClass + "-parent'> <a target='_blank' href='" + cblin._indicator.indicators[i].theme['url'] + "'></a><div class='mouseMove " + locationClass + " draw_" + cblin._indicator.indicators[i].indicatorID + " '></div></div>");
                }
                api.getData('GetIndicatorData', {
                    ...
                    if (cblin._indicator.indicators[i].indicatorType === 'Text') {
                        ...
                        draw.echart(handleKeyValues[i](this.processData, cblin._indicator.indicators[i].indicatorName, cblin._indicator.indicators[i].type), i)
                    }
                    if ( cblin._indicator.indicators[i].mapType === 'bar') {
                        this.callBackMapData.push(callBackMap(this.processData, cblin._indicator.indicators[i].mapType, _level, _code))
    ...
    var callBackMap = function(_count, _data, _level, _code) {
        console.log(_count)
