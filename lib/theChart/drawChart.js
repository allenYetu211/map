define(["jquery"], function(j, e) {
    ...
    init: function() {
      ...
      for (let i = 0; i < this._indicator.indicators.length; i++) {
        ...
        if ($(location).find('.mouseMove').hasClass('draw_' + this._indicator.indicators[i].indicatorID)) {
          ...
          $(location).append("<div class='information " + locationClass + "-parent'> <div class='mouseMove " + locationClass + " draw_" + this._indicator.indicators[i].indicatorID + " '></div></div>");
        }
        ...
        api.getData('GetIndicatorData', {
          ...
          if (this._indicator.indicators[i].indicatorType === 'Text') {
            ...
            console.log('ssss:', handleKeyValues[i](this.processData, this._indicator.indicators[i].indicatorName, this._indicator.indicators[i].type))
          ...
          // console.log('this.processData:', this.processData , 'this._indicator.indicators:', this._indicator.indicators)
          this.callBackMapData.push(callBackMap(this.processData, this._indicator.indicators))
          
          addThemeMap(this.callBackMapData);
        })
      }
    },
    handleKeyValue: null
  ...
  var callBackMap = function (_count, _data) {
    return {
      ...
      data: _count['indicatorData'][0].data
    }
  }
  ...
  var getTransformer = function(type) {
    switch (type) {
          ...
          if (raw) {
            ...
            function _getAxig(_type, attr) {
              ...
              return raw['indicatorData'].reduce(function(arr, item) {
                ...
                //   }
                // })
                return arr.concat(item[attr])
              }, [])
            }
            console.log( 'Axing:' , _getAxig('other', 'stack'))
            switch (_type) {
              case '0':
                return {
                  title: {
                  ...
                  series: raw['indicatorData'].reduce(function(arr, item) {
                    ...
                    _items = item['data'].reduce(function(_arr, _item) {
                      ...
                      // console.log(arr.concat({name: _lable, stack: _stack, type: 'bar',  data: _items}))
                    return arr.concat({ name: _lable, stack: item['stack'], type: 'bar', data: _items })
                  }, []),
                  color: ['rgba(244,32,18,0.8)', 'rgba(233,30,99,0.8)', 'rgba(156,38,176,0.8)', 'rgba(63,81,181,0.8)', 'rgba(33,150,243,0.8)', 'rgba(2,188,212,0.8)', 'rgba(76,175,80,0.8)', 'rgba(249,227,32,0.8)', 'rgba(255,152,0,0.8)', 'rgba(96,125,139,0.8)']
                }
                break;
              ...
              case '0':
                return {
                  ...
                  xAxis: [{
                    ...
                    data: raw['indicatorData'][0]['data'].reduce(function(arr, item) {
                      return arr.concat(item['lable'])
                    }, [])
                  ...
                  series: [{
                    name: name,
                    type: 'line',
                    showSymbol: false, // 根据样式需要调整
                    hoverAnimation: false, // 同上
                    data: raw['indicatorData'][0]['data'].reduce(function(arr, item) {
                      return arr.concat(_getValue(item))
                    }, []),
                    color: ['rgba(244,32,18,0.8)', 'rgba(233,30,99,0.8)', 'rgba(156,38,176,0.8)', 'rgba(63,81,181,0.8)', 'rgba(33,150,243,0.8)', 'rgba(2,188,212,0.8)', 'rgba(76,175,80,0.8)', 'rgba(249,227,32,0.8)', 'rgba(255,152,0,0.8)', 'rgba(96,125,139,0.8)']
                ...
                return {
                  ...
                  yAxis: [{
                    ...
                    axisLabel: { //坐标轴标签相关设置,距离颜色等
                      ...
                      //formatter: '{value} °C',//标签内容内置的格式转化器比如这个表示在后面加一个c
                      textStyle: {
                        // color: "#eaeaea", //坐标轴刻度文字的颜色
                        // shadowColor: '#fff',
                        fontSize: '10px' //坐标轴刻度文字的大小
                      }
                    },
                    boundaryGap: [0, '100%'],
                    splitLine: { //坐标轴分隔线。默认数值轴显示，类目轴不显示。
      ...
      case 'Dashboard_PercentDashboard':
        return function(raw, name, _type) {
          if (raw) {
            console.log('PercentDashboard_type:', _type)
            switch (_type) {
              case '0':
                return {
                  tooltip: {
