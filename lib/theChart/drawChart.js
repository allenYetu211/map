...
var drawOn = {
    draw: function (index) {
        console.log('initTerms.processData：', initTerms.processData)
        initTerms.callBackMapData = []
        console.log(map.Layers.delLayers)
        map.Layers.delLayers()
         if ( cblin._indicator.indicators[index].mapType === 'bar' || cblin._indicator.indicators[index].mapType === 'pie') {
            // initTerms.callBackMapData.push(callBackMap(initTerms.processData, cblin._indicator.indicators[index].mapType, cblin.__level__, cblin.__code__))
...
var cblin = {
    ...
    var initTerms = {
        ...
        init: function(_level, _code = '330283103', bench = false) {
            ...
            //          $('#left-sidebar, #right-sidebar').find('.mouseMove').parents('.information').remove()
            //     }
            // console.log(cblin.fig)
            // if (cblin.fig) {
            //     console.log('',cblin.__level__ == _level)
            //     if (cblin.__level__ == _level) return 
            // }
           cblin.fig = true
            var handleKeyValues = []
            var location = ''
