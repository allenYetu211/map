...
var drawOn = {
    draw: function (index) {
        ...
        let ag = ''
         $.ajax({
                type: 'POST',
                url: "http://124.207.115.117:8090/DataShow.asmx/GetIndicatorData",
                data: {
                    indicatorID: index,
                ...
                contentType: "application/x-www-form-urlencoded",
                success: function (data) {
                    if (initTerms.callBackMapData.length >= 2) initTerms.callBackMapData.shift
                    ag = JSON.parse(data)
                    console.log(cblin._indicator.indicators[index].mapType)
                        initTerms.callBackMapData.push(
                        callBackMap(cblin._indicator.indicators[index].mapType)
                        (ag, cblin._indicator.indicators[index].mapType, cblin.__level__, cblin.__code__)
                        );
