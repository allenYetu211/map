...
define(['map', "jquery"], function(map, j) {
        ...
        init: function(_level = '0', _code = '330283103') {
            ...
            for (let i = 0; i < this._indicator.indicators.length; i++) {
                ...
                if ($(location).find('.mouseMove').hasClass('draw_' + this._indicator.indicators[i].indicatorID)) {
                    ...
                    $(location).append("<div class='information " + locationClass + "-parent'> <a href='javascript:;'></a><div class='mouseMove " + locationClass + " draw_" + this._indicator.indicators[i].indicatorID + " '></div></div>");
                }
                ...
                api.getData('GetIndicatorData', {
                    ...
                    } else if (this._indicator.indicators[i].indicatorType === 'Column_CustomAxisLabel3') {
                        draw.echart(handleKeyValues[i](testData3, this._indicator.indicators[i].indicatorName, this._indicator.indicators[i].type), i)
                    } else if (this._indicator.indicators[i].indicatorType === 'Line_FlashingLineChart') {
                        return
                    } else {
                    ...
                    if (raw) {
                        ...
                        function  sc() {
                          var _name = ''
                             for (let i = 0; i < raw['indicatorData'].length; i++) {
                                        for (let k = 0; k < raw['indicatorData'][i]['data'].length; k++) {
                                          for (let j = 0; j < raw['indicatorData'][i]['data'][k]['other'].length; j++) {
                                            if (raw['indicatorData'][i]['data'][k]['other'][j]['F_NAME'] !== undefined) {
                                              (function (i, k, j) {
                                                _name = raw['indicatorData'][i]['data'][k]['other'][j]['F_NAME']
                                              })(i, k, j)
                                            }
                                          }
                                          // if ( raw['indicatorData'][i]['data'][k]['other'][k]['F_NAME'] !== undefined){
                                          //   _name = item['data'][i]['other'][k]['F_NAME']
                        ...
                        function  testDs() {
                          ...
                          cms =  raw['indicatorData'][0]['data'].reduce(function (_arr, _item) {
                                ...
                                // console.log('_name', _item['other'])
                                // console.log('_stack', _item['data'])
                                // console.log("_item['data']:", _item['data'])
                            ...
                            case '1':
                                return {
                                    ...
                                    series: raw['indicatorData'].reduce(function(arr, item) {
                                        ...
                                        console.log("raw['indicatorData']:", raw['indicatorData'])
