define(["jquery"], function(j, e) {
  ...
  var initTerms = {
    ...
    cleartButton: function(_id, _name) {
      // $('#bottom-sidebar').append('<button class="button_d" data-id=' + _id + '>' + '<h2>' + _id + '</h2>' + '<h4>' + _name + '<span></br>' + 'Stage ' + _id + '</span>' + '</h4>' + '</button>')
      ...
      for (let i = 0; i < this._indicator.indicators.length; i++) {
        ...
        api.getData('GetIndicatorData', {
          ...
          if (this._indicator.indicators[i].indicatorType === 'Text') {
            ...
            draw.echart(handleKeyValues[i](testData2, this._indicator.indicators[i].indicatorName, this._indicator.indicators[i].type), i)
          } else {
