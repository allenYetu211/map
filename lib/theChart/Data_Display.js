define("chartInformations", ['map', "jquery", "userservice", "translatePopup", 'local', 'api', 'common'], function(map, j, us, c, l, _API, common) {
  ...
  $('.map1').on('click', function() {
    ...
    $('#slide-pop, .filedCondition').removeClass('dakr')
  ...
  $('.map2').on('click', function() {
    ...
    $('#slide-pop, .filedCondition').addClass('dakr')
  ...
  var sendRequest = {
    ...
    GetSurveyLevel: function(minscale) {
      ...
      _API._G('DataShow.asmx/GetDataByCatalogID', {
        ...
        storage.menuListTrdata.datas.forEach(function(_val) {
          ...
          _ct +=
            ...
            '   <li class="js_ontp" data-tn="' + _val.tableName + '" data-tp="' + _val.tilePath + '">' + _val.name +
    ...
    tsData: function() {
      ...
      for (var i = 0; i < storage.chartData.length; i++) {
        ...
        $(location).append("<div class='information " + sizeType + "-parent'>" +
          "<button data-sizemap=" + _t.indiatorInfo.size_type + " data-name=" + _t.indiatorInfo.name + " data-type=" + _t.indiatorInfo.type + " data-index=" + _t.indiatorInfo.id + " class=" + is_map + "></button> " + introduce +
  ...
  $('._rotate').on('click', function () {
    var target = $('#slide-pop').find('.active')
