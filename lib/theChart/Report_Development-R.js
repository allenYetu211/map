define('dataLaySth', ['jquery', 'local', 'api'], function(jq, l, _API) {
  ...
  $('._Deletelaysth').on('click', function() {
    var _html = $('li._j_getInfo.active').html()
    $('.al-popup-prompt').show().find('.al-prompt-informations').html('确认删除 "' + _html + '" 表?')
    $('.deleteLaysth').show()
  })
  ...
  $('body').on('focus', '._zdPopupSelect input', function(e) {
  ...
  $('body').on('keyup', '._selectInfo', function(e) {
  ...
  var iframeCmita = {
    _selectedRange: function(location) {
      ...
      if (!storage.addTable && !storage.mouseUp) {
        ...
        $('#location').val(location.minLetter)
      }
    }
  }
  ...
  $(window).on('load', function() {
    setTimeout(function() {
      if ($('#viewinfo-sheetFile').length !== 0) {
        $('#viewinfo-sheetFile')[0].contentWindow.ins_displaySheet(true, true, false)
      ...
      _fillinfo.reportInfo.forEach(function(_val) {
        ...
        switch (_val.type) {
          ...
          case 'number_mulDic':
            ...
            var _values = _val.value.split(',')
            for (var key in _val.values) {
              var flag = true
              _values.forEach(function(_newValue) {
                if (_val.values[key] == _newValue) {
                  infoData = [key]
