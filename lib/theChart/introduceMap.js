define("mapIntroduce", ['map', "jquery", "userservice", "chartInformations", "sqlBuilder", "queryBuilder"], function(m, j, us, ch, sq, qb) {
    ...
    var sendRequest = {
        ...
        GetFieldInfo: function(_vs, _in) {
            ch.api.getData('GetFieldInfo', {
                egFields: storage.tableField[_vs].egField
            }, function(data) {
                ...
                storage.fieldInfoData.push(data.fieldInfo[0])

                if ($('.select_cf').length == storage._requertCount) {
                    if (typeof sqlBuilder == 'function') {
                        sqlBuilder = new SqlBuilder('sql-builder')
                        sqlBuilder.init(storage.fieldInfoData)
                    }
                    sqlBuilder.setFilters(storage.fieldInfoData)
                }
                storage.fields.push(data.fieldInfo[0].id)
            })
        },
    ...
    var embellish = {
        ...
        introduce: function() {
            var themeAnalysis = ''
            console.log(storage.introduceDATA)
