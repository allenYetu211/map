...
define('User_Manage', ['jquery','local', 'api'], function(jq, l, _API) {
    ...
    var sendRequest = {
        ...
        GetDistrict:function(column,code){
        	_API._G('UserManage.asmx/GetDistrict', {
                ...
                if(column == 'F_PROVINCE'){
                	//市
                	var city = $('#userTableList table ._addTr td:eq(3)');
					var span=$('<span></span>');
                	...
                	//区
                	var county = $('#userTableList table ._addTr td:eq(4)');
					var span=$('<span></span>');
	            	var select=$('<select></select>');
	            	for(var key in data.values){
	                	select.append($('<option data-selectIndex="'+ key +'" selected="selected">'+ data.values[key] +'</option>'));
        ...
        _GetDistrict:function(column,code,_this){
        	_API._G('UserManage.asmx/GetDistrict', {
                column:column,
                code:code
            }, function(data){
                if(column == 'F_PROVINCE'){
                	//市
                	_this.find('td:eq(3)').html('');
                	_this.find('td:eq(4)').html('');
                	var city = _this.find('td:eq(3)');
					var span=$('<span></span>');
	            	var select=$('<select></select>');
	            	for(var key in data.values){
	                	select.append($('<option data-selectIndex="'+ key +'" selected="selected">'+ data.values[key] +'</option>'));
...
//      },1000)
    ...
    //三级联动 省
	$('body').on('click','#userTableList table ._addTr td:eq(2)',function(){
		$('#userTableList table ._addTr td:eq(3)').html('');
		$('#userTableList table ._addTr td:eq(4)').html('')
		...
		var dataSelectindex = $(this).find('select option:eq(0)').attr('data-selectindex')
		var txt = $(this).find('select option:eq(0)').val()
		$(this).find('span').html(txt).attr('data-selectindex',dataSelectindex)
	})
    //三级联动  市
    $('body').on('click','#userTableList table ._addTr td:eq(3)',function(){
    	$('#userTableList table ._addTr td:eq(4)').html('')
...
//      IsNull();
		function IsNull(){
			if($('tr._addTr td').eq(0).find('span').html() == '' || $('tr._addTr td').eq(15).find('span').html() == ''){
				isSuccess('不能为空')
				RenderUserTable()
				flag = false
				return false
        ...
        if(storage.userFlag=='userGroup'){
            ...
            if(storage.notC&&flag){
                var userGroupsArr=[];
                $('#userTableList .change').each(function(i,v){
                    MsgTr(v);
                })
                sendRequest.UserGroupOp(userGroupsArr);
            }
            ...
            for(var i in storage.user.column){
                switch (storage.user.columnPro[i].type){
                    ...
                    case 'number_dic':
                        ...
                        _obj[storage.user.column[i].egField]=$($(v).children().get(i)).find("option:selected").attr("data-selectindex");
                        break;
                    case 'number_mulDic':
    ...
    $('.changeListData').on('click', function() {
        ...
        storage.change = true
        if(storage.change){
        	var _deleteTr = $('#userTableList table ._deleteTr');
        	_deleteTr.each(function(i,v){
        		//省
        		var _this = $(this);
        		$(v).find('td:eq(2)').click(function(){
					var column = $("#userTableList table tr:eq(0) th:eq(2)").attr('data-egfield');
					var code = '';
					var span =_this.find('td:eq(2)').find('span');
					_this.find('td:eq(2)').find('select').change(function(){
						setTimeout(function(){
							code = span.attr('data-selectindex');	
							sendRequest._GetDistrict(column,code,_this);
						},0)
					})
        		})
        		$(this).find('td:eq(3)').click(function(){
        			var column = $("#userTableList table tr:eq(0) th:eq(3)").attr('data-egfield');
					var code = '';
					var span =_this.find('td:eq(3)').find('span');
					var dataSelectindex = _this.find('td:eq(3) option:eq(0)').attr('data-selectindex');
					var txt = _this.find('td:eq(3) option:eq(0)').html();
					span.html(txt).attr('data-selectindex',dataSelectindex);
					_this.find('td:eq(3)').find('select').blur(function(){
						setTimeout(function(){
							code = span.attr('data-selectindex');
							sendRequest._GetDistrict(column,code,_this);
						},0)
	...
	$('body').on('click','.user-left-bar ._user ul li',function(){
		$('.addListData').show();
		$('.changeListData').show();
		$('.deleteListData').show();
	})
...
//  $('body').on('blur', '#userTableList td select', function(){
//      var replaceNew=$(this).val()
//      var dataSelectindex=$(this).find("option:selected").attr("data-selectindex");
//      $(this).prev().html(replaceNew).attr("data-selectindex",dataSelectindex);
//      $(this).hide()
...
//    })
	//角色列表多选框
	$('body').on('click','#userTableList>ul li.parentNode>ul input',function(){
		var children = $(this).next();
        var isChecked = $(this).prop("checked");
        if(children){
            if(isChecked){
                if($(this).attr('isChild') == 'true'){
                	$(this).parent().parent().find('ul input').prop('checked',"checked").attr('isSelect',true)
                }
            }else{
                if($(this).attr('isChild') == 'true'){
                	$(this).parent().parent().find('ul input').removeProp('checked').attr('isSelect',false)
                }
            }
        }
        var checkedCount = $(this).parent().parent().parent().find(">li>label>input:checked").length;
        var count = $(this).parent().parent().parent().find(">li>label>input").length;
        if(checkedCount == count){
            $(this).parent().parent().parent().prev().find(">input").prop("checked","checked");
        }else{
            $(this).parent().parent().parent().prev().find(">input").removeProp("checked");
        }
        var allCount = $('#userTableList ul li.parentNode input').length;
        var allCheckedCount = $('#userTableList ul li.parentNode input:checked').length;
        if(allCheckedCount == allCount-1){
        	$('#userTableList ul li.parentNode input:eq(0)').attr('isSelect',true).prop("checked","checked")
        }
        
	})
	$('body').on('click','#userTableList ul li.parentNode input:eq(0)',function(){
        var isChecked = $(this).prop("checked");
        var inp = $('#userTableList ul li.parentNode input');
		if(isChecked){
			$(this).attr('isSelect',true);
			inp.each(function(i,v){
				$(v).attr('isSelect',true).prop('checked','checked')
			})
		}else{
			$(this).attr('isSelect',false)
			inp.each(function(i,v){
				$(v).attr('isSelect',false).removeProp('checked')
			})
		}
	})
	$('body').on('click','#userTableList>ul li.parentNode>ul input',function(){
        ...
        inp.each(function(i,v){
        	var atr = $(v).attr('isSelect');
        	arr.push(atr)
        })
        if(arr.some(function(item){
        	return item == 'true'
        })){
        	$(this).parent().parent().parent().parent().find('input:eq(0)').prop('checked','checked');
        	$(this).parent().parent().parent().parent().find('input:eq(0)').attr('isSelect',true);
        }
        
        if(arr.every(function(item){
        	return item == 'false'
        })){
        	$(this).parent().parent().parent().parent().find('input:eq(0)').removeProp('checked');
        	$(this).parent().parent().parent().parent().find('input:eq(0)').attr('isSelect',false);
        }
        
        var res = [];
        var allInp = $(this).parent().parent().parent().parent().parent().find('input')
        allInp.each(function(i,v){
        	var atr = $(v).attr('isSelect');
        	res.push(atr)
        })
        if(res.some(function(item){
        	...
        	$('#userTableList').find('input:eq(0)').prop('checked','checked');
        	$('#userTableList').find('input:eq(0)').attr('isSelect',true);
        }
        if(res.every(function(item){
        	return item == 'false'
        })){
        	$('#userTableList').find('input:eq(0)').removeProp('checked');
        	$('#userTableList').find('input:eq(0)').attr('isSelect',false);
        }
