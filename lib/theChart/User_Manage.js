...
define('User_Manage', ['jquery','local', 'api'], function(jq, l, _API) {
    ...
    var sendRequest = {
        ...
        UserGroupOp: function() {
            console.log(typeof storage.UserGroupOpType)
            var _obj = {}
            _obj['userGroups']=storage.userGroupsArr;
            console.log({
        ...
        // 1.10.4 用户组下的用户接口
        GetUserByGroupID: function(a) {
            _API._G('UserManage.asmx/GetUserByGroupID', '', function(data){
    ...
    $('body').on('click', '._deleteTr', function() {
        ...
        $('._deleteTr').each(function(i,v){
            if(v.className=='_deleteTr active'){
                ...
                storage.userGroupsArr.push(_obj);
            }
        })
    })
    ...
    $('.addListData').on('click', function() {
        ...
        var tr=$('<tr class="_addTr" data-id="'+ (_dataid+1)+'"></tr>');
