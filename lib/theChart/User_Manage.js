...
define('User_Manage', ['jquery','local', 'api'], function(jq, l, _API) {
    ...
    var storage = {
        ...
        notA: false,
        notC: false,
        FieldOpType: '',   //操作类型（0：新增、 1：修改、 2：删除） 新增时userGroupID传-1
        deleteAry: []      //存储删除索引
    }
    ...
    var sendRequest = {
        ...
        //1.10.2 用户组操作接口
        UserGroupOp: function(userGroups,opType) {
    ...
    $('body').on('click', '._deleteTr', function() {
        ...
        storage.deleteAry.push($(this).index() - 1)   //点击的那一行的索引值
        console.log(storage.FieldOpType)
