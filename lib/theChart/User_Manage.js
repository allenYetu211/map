...
define('User_Manage', ['jquery','local', 'api'], function(jq, l, _API) {
    ...
    var sendRequest = {
        ...
        //1.10.2 用户组操作接口
        UserGroupOp: function() {
            console.log(storage.UserGroupOpType)
            var _obj = {}
            var _ary = []

            _obj['userGroups']=_ary;
            console.log(_obj)
            _API._G('UserManage.asmx/UserGroupOp', {
                userGroups:JSON.stringify(_obj),
                opType:storage.UserGroupOpType
            }, function(data){
                // storage.userGroup=data.userGroup;
                console.log(data)
            })
                ...
                switch (Pro.type){
                    ...
                    case 'number_mulDic':
                        ...
                        for(var l in Pro.values){
                            if(Pro.values[l]==storage.userGroup.data[j][k].value){
    ...
    $('.addListData').on('click', function() {
        ...
        var ct = ''
        ct += '<tr class="_addTr" data-id= "' + (_dataid + 1) + '">'
        // var _vs = storage.bygroupData.field.data
        storage.userGroup.columnPro.forEach(function(_val, _int) {
            // _pro : Object {type: "string"}
            var _pro = storage.userGroup.columnPro[_int]
            ...
            switch (_val.type) {
                ...
                case 'number_dic':
                    ...
                    for (var key in _pro.values) {
                        if (_int == storage.userGroup.columnPro.length - 1) {
    ...
    $('body').on('click', '.mulDic_p', function() {
        ...
        console.log($(this).parents('td').find('span'))
        $(this).parent().hide()
    ...
    // $('body').on('blur', '#userTableList td .mulDic_input', function() {
    //     var replaceNew = $(this).val()
    //     $(this).parent().hide()
