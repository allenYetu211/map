    ...
    var sendRequest = {
        ...
        GetFuncByRoleID: function(roleID) {
            _API._G('UserManage.asmx/GetFuncByRoleID', {
                ...
                console.log('1.10.8',data)
                storage.fun=data.function;
                FuncByRole();
            })
        },
        // 1.10.9 保存功能权限接口
        SaveFunction: function(roleID) {
            ...
            if(storage.fun[i].pId==id){
                if(storage.fun[i].isChild){
                    ...
                    if(storage.fun[i].isSelect){
                        var label=$('<label style="color: #0088EB">'+ storage.fun[i].name +'<input type="checkbox" checked></label>')
                        li.append(label)
                    }else{
                        var label=$('<label style="color: #0088EB">'+ storage.fun[i].name +'<input type="checkbox"></label>')
                        li.append(label)
                    }
                    var id=storage.fun[i].id;
                    var nextFn=recursionFuncByRole(id);
                    li.append(nextFn);
                }else {
                    if(storage.fun[i].isSelect){
                        var li=$('<li><label>'+ storage.fun[i].name +'<input type="checkbox" checked></label></li>')
                    }else{
                        var li=$('<li><label>'+ storage.fun[i].name +'<input type="checkbox"></label></li>')
                    }
                }
            }
            ul.append(li);
