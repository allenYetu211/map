...
define('User_Manage', ['jquery','local', 'api'], function(jq, l, _API) {
    ...
    //------用户管理-----
    $('#user-left-bar-bar-bar-bar-bar h4._user').toggle(function(){
        $(this).find('img').addClass('active');
        $('#user-left-bar-bar-bar-bar-bar ul._user').slideDown();
    },function(){
        $(this).find('img').removeClass('active');
        $('#user-left-bar-bar-bar-bar-bar ul._user').slideUp();
    })
    $('#user-left-bar-bar-bar-bar-bar h4._user').click(function(){
        sendRequest.GetUserGroup('user');
    })

    //-----用户组管理-----
    $('#user-left-bar-bar-bar-bar-bar span._users').click(function(){
        sendRequest.GetUserGroup('users');
    })

    //-------角色管理------
    $('#user-left-bar-bar-bar-bar-bar h4._role').toggle(function(){
        $(this).find('img').addClass('active');
        $('#user-left-bar-bar-bar-bar-bar ul._role').slideDown();
    },function(){
        $(this).find('img').removeClass('active');
        $('#user-left-bar-bar-bar-bar-bar ul._role').slideUp();
    })
    $('#user-left-bar-bar-bar-bar-bar h4._role').click(function(){
        sendRequest.GetRole('role');
    })
    $('#user-left-bar-bar-bar-bar-bar span._role').click(function(e){
        e.stopPropagation();
