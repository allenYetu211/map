define("chartInformations", ['map', "jquery", "userservice"], function(map, j, us) {
    ...
    $('.map1').on('click', function() {
        $('#top-sidebar, #left-sidebar, #right-sidebar, .popupMenuInformation, .formData').removeClass('dark'),
            $('.logo img, .formData img').attr('src', 'images/chart/logo@2x.png')
    })

    $('.map2').on('click', function() {
        $('#top-sidebar,#left-sidebar, #right-sidebar, .popupMenuInformation, .formData').addClass('dark'),
    ...
    var mapCall = {
            callLevelCode: function(_level, _code) {
                ...
                sendRequest.GetIndicators(_level)
            }
        }
        /**
         * 发送请求
        ...
        GetDataByCatalogID: function(_id, _t) {
            api.getData('GetDataByCatalogID', {
                ...
                storage.menuListTrdata.datas.forEach(function(_val) {
                        console.log(_val)
                        var _ct = '<ul><li class="js_ontp" data-tn="' + _val.tableName + '" data-tp="' + _val.tilePath + '">' + _val.name + '<div class="right-options"><span class="clickChoice" data-tn="' + _val.tableName + '"  data-tp="' + _val.tilePath + '"><img src="images/chart/icon_mapsort@2x.png" alt=""></span><span ><img src="images/chart/icon_mapsort@2x.png" alt=""></span> <span class=""><img src="images/chart/icon_mapfold@2x.png" alt=""></span></div></li></ul>'
                        _t.parents('li').append(_ct)

                    })
                    //    storage.menuListTrdata
            })
    ...
    var toDealWith = {
        ...
        concatMenu: function() {
            var _im = ''
            var assignObj = ''
            console.log('storage.menuList.catalog:', storage.menuList.catalog)
            console.log('storage.menuListArr:', storage.menuListArr)
            ...
            console.log('storage.menuNewData:', storage.menuNewData)
            draw.menuList();
        },
        /**
         * 错误代码 ^^^
    ...
    var drawOn = {
        draw: function(_id) {
            var mapInArroy = []
            var mapChart = '';
            console.log('storage.chartData:', storage.chartData)
