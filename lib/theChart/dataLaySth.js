define('dataLaySth', ['jquery', 'map', 'chartInformations', 'mapIntroduce'], function(jq, map, ch, mi) {
    ...
    $('._Deletelaysth').on('click', function() {
        ...
        storage._ob = {}
        storage._ob['reportid'] = storage.reportID
        console.log(storage._ob)
        sendRequst.ReportOp()
    })
    ...
    var iframeCmita = {
        _selectedRange: function(location) {
            if (!storage.addTable) {
                storage.fieldOp['F_FIELDID'] = '-1'
                storage.tabalopType = '0'
                sendRequst.GetFieldInfo('-1', location)
    ...
    var sendRequst = {
        ...
        ReportOp: function(_ob) {
            ch.api.getDevelopment('ReportOp', {
                ...
                if (!data.success) {
                    ...
                    var ct = '<li data-reportid= ' + data.reportID + '  class="_j_getInfo">' + _val + '</li>'
                    $('._c_open').append(ct)
                    $('.popupAddChange').hide()
                }
        ...
        GetFieldInfo: function(_fieldID, location) {
        ...
        ReportFieldOp: function(add) {
            ...
            ch.api.getDevelopment('ReportFieldOp', {
                ...
                $('#viewinfo-sheetFile')[0].contentWindow.ins_make($('#location').valu())
