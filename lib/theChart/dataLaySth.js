define('dataLaySth', ['jquery', 'map', 'chartInformations', 'mapIntroduce'], function(jq, map, ch, mi) {
    ...
    $('body').on('change', '._j_fieldGroup', function() {
    ...
    $('body').on('change', '.PopupAdd-ZD select ', function() {
        // console.log($(this))
        // $(this).parent('.select').data('selected', $(this).data(flicid))
    ...
    var sendRequst = {
            ...
            ch.api.getDevelopment('ReportOp', {
                ...
                if (!data.success) {
                    ...
                    if (storage.reportType == 0) {
                        ...
                        console.log($('._j_getInfo.active').parents('div').index() == _type - 1)
                        console.log(_type - 1)
        ...
        GetFieldOfGroup: function(group, _t) {
            ch.api.getDevelopment('GetFieldOfGroup', {
                ...
                if (data.field.length <= 0) {
                    _t.parents('._zdPopupSelect').next().find('select').html('')
                } else {
        ...
        ReportFieldOp: function(add) {
            ...
            ch.api.getDevelopment('ReportFieldOp', {
                ...
                if (storage.tabalopType == 2) {
                    ...
                    var _info = $('.PopupAdd-ZD').find('label[data-eg=FK_FIELD]').next().find('option:selected').text()
        ...
        CreateData: function() {
            ch.api.getDevelopment('CreateData', {
                reportID: storage.reportID
            }, function(data) {
                if (!data.success) {
                    $('.al-popup-prompt').show().find('.al-prompt-informations').html(data.msg)
                ...
                switch (_val.type) {
                    ...
                    case 'number_mulDic':
                        ...
                        for (var key in _val.values) {
                            if (_val.values[key] == _val.value) {
                                infoData = [key]
                                _ctInfo += '<li class="_j_selectLab _selected" ><input  id="' + _val.egField + key + '" type="checkbox" checked><label data-info="' + key + '" for="' + _val.egField + key + '">' + _val.values[key] + '</label></li>'
                                ...
                                _ctInfo += '<li class="_j_selectLab"><input id="' + _val.egField + key + '" type="checkbox"><label for="' + _val.egField + key + '"  data-info="' + key + '">' + _val.values[key] + '</label></li>'
                            }
                        }
                        _ctInfo += '</ul></div>'
                        break;
