define('dataLaySth', ['jquery', 'map', 'chartInformations', 'mapIntroduce'], function(jq, map, ch, mi) {
  ...
  var sendRequst = {
    ...
    sendFrom: function() {
      ...
      fromInfor.append('file', $('#fileData')[0].files[0])
        //   fromInfor['file'] = $('#fileData')[0].files[0]
      console.log(fromInfor)
      ch.api.postFormData(fromInfor, function(data) {
        sendRequst.ReportOp()
  ...
  var rendering = {
    ...
    fieldInfo: function() {
      ...
      storage.fieldInfo.reportInfo.forEach(function(_val) {
        ct += '<div class="_zdPopupSelect"><label data-eg="' + _val.egField + '">' + _val.chField + '</label>'
        switch (_val.type) {
          case 'string':
            ct += '<input id="location" type="text">'
            break;
          ...
          default:
            ...
            if (_val.egField == 'FK_FIELD_GROUP') {
              ct += '<select data-selected="' + src + '" class="_j_fieldGroup"> ' + info
            } else if (_val.egField == 'FK_REPORT') {
              ct += '<select data-selected="' + src + '" class="_j_report">' + info
            } else {
              ct += '<select data-selected="' + src + '">' + info
            }
            break;
        }
