...
define('System_Setting', ['jquery','local', 'api'], function(jq, l, _API) {
    ...
    $('.serverListData').on('click', function() {
    	//修改添加span
    	function add(){
    		var changeTr = $('#systemTableList ._changeTr');
    		var sel = changeTr.find('td:eq(4) select');
    		var txt = sel.val();
			var dataSelectindex = sel.find('option:contains('+ txt +')').attr('data-selectindex');
			$('<span data-selectindex='+ dataSelectindex +'>'+ txt +'</span>').insertBefore(sel)
    	}
    	add()
        //底图管理
        function MsgTr1(v){
    ...
    $('body').on('click','.system-left-bar li',function(){
        $('.system-left-bar ul li').removeClass('active');
        $(this).addClass('active');
        setTimeout(function(){
        	var size = $('#systemTableList table tr').length;
        	if(size == 1){
        		isSuccess('暂无数据')
        	}
        },1000)
...
//  })
    ...
    $('body').on('click','.mulDic_ul li',function(){
    	$('.mulDic_ul').show();
    })
...
//      return false;
//  })
    $('body').on('mouseleave','.mulDic_ul',function(){
    	$('#systemTableList').click(function(){
    		$('.mulDic_ul').parent().prev().show();
    		$('.mulDic_ul').hide();
    		$('.mulDic_ul').parent().hide();
//	  		mulDic_btn=true;
//      	return false;
    	})
    })
	$('body').on('click','._addTr',function(){
		...
		td.each(function(i,v){
			var _v = $(v)
			//$(v).find('select option:eq(0)').attr('selected','selected');
			$(v).click(function(){
				_v.find('span').html(_v.find('option[selected]').html())
				_v.find('span').attr('data-selectindex',_v.find('span').attr('data-selectindex'))
			})
		})
	})
	$('body').on('click','._changeTr td:eq(4)',function(){
		var txt = $(this).prev().find('span').html();
		if(txt == '矢量'){
			$('<span><span>').insertBefore($(this).find('<select></select>'))
		}
	})
})