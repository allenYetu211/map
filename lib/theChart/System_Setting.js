...
define('System_Setting', ['jquery','local', 'api'], function(jq, l, _API) {
            ...
            for(var k in storage.baseMap.data[j]){
                ...
                switch (Pro.type){
                        ...
                        if(k==3||k==4){
                            for(var x in Pro.values){
                                if(Pro.values[x]==storage.baseMap.data[j][k].value){
                                    var span=$('<span data-selectindex="'+ x +'">'+ storage.baseMap.data[j][k].value +'</span>')
                                }
                            }
                        }else{
                            var span=$('<span>'+ storage.baseMap.data[j][k].value +'</span>')
                    ...
                    case 'number_mulDic':
                        ...
                        //     if((storage.baseMap.data[j][k].value.indexOf(Pro.values[l]))!==-1){
    ...
    $('.serverListData').on('click', function() {
        // console.log(storage.surveyOpType)
    ...
    // sendRequest.GetSurveyByCategory(_categoryID);
    $('body').on('click','.system-left-bar .catalogue li span',function(){
        storage.notA = false
        storage.notC = false
        storage.notD = false
        $('.changeListData, .deleteListData').removeClass('active')
    ...
    // sendRequest.GetSurveyByCategory(_MapGroupID);
    $('body').on('click','.system-left-bar .base-map li span',function(){
        storage.notA = false
        storage.notC = false
        storage.notD = false
        $('.changeListData, .deleteListData').removeClass('active')
        var MapGroupID=$(this).attr('MapGroupID')
