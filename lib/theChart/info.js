    ...
    var sendRequest = {
        ...
        GetSurveyData: function(categoryID,surveyID) {
            ch.api.getCetCatgory('GetSurveyData', {
                ...
                console.log('1.6.5 获取调查数据接口:',data);
            ...
            if(storage.surveyData[i].pId==id){
                if(storage.surveyData[i].ischild){
                    ...
                    if(storage.surveyData[i].isSelect==true){
                        var li=$('<li><lable><input type="checkbox" checked dataCode="'+ storage.surveyData[i].dataCode +'">'+ storage.surveyData[i].dataName +'</lable></li>')
                    }else{
                        var li=$('<li><lable><input type="checkbox" dataCode="'+ storage.surveyData[i].dataCode +'">'+ storage.surveyData[i].dataName +'</lable></li>')
                    }
                }
            }
            ul.append(li);
