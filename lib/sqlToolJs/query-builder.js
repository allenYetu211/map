...
<select class="form-control {{? it.operators.length === 1 }}hide{{?}}" name="{{= it.rule.id }}_operator"> \
  ...
  {{? optgroup !== null }}</optgroup>{{?}} \
    ...
    QueryBuilder.prototype.getRuleInput = function(rule, value_id) {
        ...
        if (typeof filter.input == 'function') {
            ...
            switch (filter.input) {
                ...
                h+= '></textarea>';
	            */
                    var selectHtml = '<select class="form-control" style="width:302px;margin-left:0px" onchange="this.parentNode.nextSibling.value=this.value"' + (filter.multiple ? ' multiple' : '') + '>';
                    if (filter.placeholder) {
                        selectHtml += '<option value="' + filter.placeholder_value + '" disabled selected>' + filter.placeholder + '</option>';
                    }
                    Utils.iterateOptions(filter.values, function(key, val) {
                        selectHtml += '<option value="' + key + '">' + val + '</option> ';
                    });
                    selectHtml += '</select>';
                    var inputHtml = '<textarea class="form-control" name="' + name + '"';
                    if (filter.size) inputHtml += ' cols="' + filter.size + '"';
                    if (filter.rows) inputHtml += ' rows="' + filter.rows + '"';
                    if (validation.min !== undefined) inputHtml += ' minlength="' + validation.min + '"';
                    if (validation.max !== undefined) inputHtml += ' maxlength="' + validation.max + '"';
                    if (filter.placeholder) inputHtml += ' placeholder="' + filter.placeholder + '"';
                    inputHtml += ' style="position:absolute;left:0px;border-right-color:white;width:283px;height:34px"';
                    inputHtml += '></textarea>';
                    h += '<div style="position:relative;"><span style="overflow:hidden;">' + selectHtml + '</span>' + inputHtml + '</div>';
                    break;
                ...
                default:
                    switch (QueryBuilder.types[filter.type]) {
					   ...
					   h+= '<input class="form-control" type="number" name="' + name + '"';
                        ...
                        h+= '>';
					   */
                            var selectHtml = '<select class="form-control" style="width:175px;margin-left:0px" onchange="this.parentNode.nextSibling.value=this.value"' + (filter.multiple ? ' multiple' : '') + '>';
                            if (filter.placeholder) {
                                selectHtml += '<option value="' + filter.placeholder_value + '" disabled selected>' + filter.placeholder + '</option>';
                            }
                            Utils.iterateOptions(filter.values, function(key, val) {
                            ...
                            selectHtml += '</select>';
                            var inputHtml = '<input class="form-control" type="number" name="' + name + '"';
                            if (validation.step !== undefined) inputHtml += ' step="' + validation.step + '"';
                            if (validation.min !== undefined) inputHtml += ' min="' + validation.min + '"';
                            if (validation.max !== undefined) inputHtml += ' max="' + validation.max + '"';
                            if (filter.placeholder) inputHtml += ' placeholder="' + filter.placeholder + '"';
                            if (filter.size) inputHtml += ' size="' + filter.size + '"';
                            inputHtml += ' style="position:absolute;left:0px;border-right-color:white;width:155px;">';
                            h += '<div style="position:relative;"><span style="overflow:hidden;">' + selectHtml + '</span>' + inputHtml + '</div>';
                            break;
    ...
    QueryBuilder.prototype.selectValue = function(selectName, inputName) {
        alert(1);
        selectName = 'aaa';
        inputName = 'sql-builder_rule_0_value_0';
        var txt = document.getElementById('sel');
        var select = document.getElementById('txt');
