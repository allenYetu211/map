document.write('<link href="sheetPanel/range/Range.css" rel="stylesheet">');
document.write('<input placeholder="0 - 200" style="display: none" type="number" min="0" max="200" step="10" value="100" data-color="#1C8EC5" class="srs">');

function Range(valueChangedFn, zoom) {
    init(valueChangedFn, zoom);
}

function init(valueChangedFn, zoom) {
    !
        function ($) {
            var t;
            return t = function () {
                function t(t) {
                    ...
                    }(this)), this.setValue(zoom);this.positionThumb(zoom);
                }
